...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    case K_COPY:
        if (record->event.pressed) {
            ...
            register_code(C(KC_C));
        } else {
            unregister_code(C(KC_C));
    ...
    case K_PASTE:
        if (record->event.pressed) {
            ...
            register_code(C(KC_V));
        } else {
            unregister_code(C(KC_V));
    ...
    case K_CUT:
        if (record->event.pressed) {
            ...
            register_code(C(KC_X));
        } else {
            unregister_code(C(KC_X));
	...
	case K_UNDO:
        if (record->event.pressed) {
            ...
            register_code(C(KC_Z));
        } else {
            unregister_code(C(KC_Z));
	...
	case K_REDO:
        if (record->event.pressed) {
            ...
            register_code(C(KC_Y));
        } else {
            unregister_code(C(KC_Y));
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [layer3] = LAYOUT(
                 K_COPY,   K_CUT,   K_PASTE, TO(layer0),
                 KC_PMNS,   KC_WH_U,   KC_PPLS, KC_BSPC,
        KC_NO, KC_WH_L,   KC_ESC,   KC_WH_R, KC_TAB,
        KC_NO,  KC_NO, KC_NO, KC_NO, KC_NO
    )
};
...
bool encoder_update_user(uint8_t index, bool clockwise) {
  if (index == 0) { /* First encoder below the controller */
    ...
    if (clockwise) {
      tap_code16(C(KC_PMNS)); /*ZOOM OUT*/
    } else {
      tap_code16(C(KC_PPLS)); /*ZOOM IN*/
    }
  }
    return true;
}
